...
{
	// ANCHOR_LETTER_CRASH_FIX_START - was: Data->Memo[Data->MemoSize] = '\0'; (wrote 1 byte past
	// the actually-received packet whenever MemoSize == the letter's real length, landing on
	// CPacket's 0xFD guard byte -> debug-CRT assert "MemBlock == 0xFD" in wsctlc_addon.h. Never
	// write into Data->Memo in place; clamp MemoSize to what was actually received (packet's own
	// declared size, from the C2 header's SizeH/SizeL) before anything below reads Memo[].
	LPFS_LETTER_TEXT Data = (LPFS_LETTER_TEXT)ReceiveBuffer;
	WORD wPacketSize = ((WORD)Data->Header.SizeH << 8) | (WORD)Data->Header.SizeL;
	int iMaxMemo = (int)wPacketSize - (int)offsetof(FS_LETTER_TEXT, Memo);
	...
	// ANCHOR_LETTER_CRASH_FIX2_START - was: strncpy(szText, (const char*)Data->Memo, 1000); szText[1000]='\0';
	// (read past the actually-received Memo bytes whenever MemoSize < 1000 - same class of bug
	// as the in-place write above, just on the read side). Data->MemoSize is already clamped above.
	memcpy(szText, (const char *)Data->Memo, Data->MemoSize);
	szText[Data->MemoSize] = '\0';
	// ANCHOR_LETTER_CRASH_FIX2_END
