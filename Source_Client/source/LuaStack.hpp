...
Ported into RebirthProjectMU from the "RoxGaming Main 5.2 - 60 FPS UPDATE"
source pack (source/LuaStack.hpp there). Two changes from upstream, both
because that pack links LuaJIT (which always reports LUA_VERSION_NUM==501,
even in 5.2-compat mode) while this client vendors real Lua 5.2:
  1. Dropped the `#include <luajit\...>` block - this client already has
     lua_State/luaL_* project-wide via stdafx.h's "lua/lua.hpp".
  2. luaL_mysetfuncs was only ever defined for LUA_VERSION_NUM <= 501 (the
     branch LuaJIT always takes); real 5.2 (LUA_VERSION_NUM 502) has no such
     function and would fail to compile. Mapped it to the real luaL_setfuncs
     for that case - same signature.
*/
...
inline void luaL_mysetfuncs(lua_State* L, const luaL_Reg* l, int nup) {
...
#else
#define luaL_mysetfuncs luaL_setfuncs
#endif
...
namespace LUAAA_NS
{
	...
	{
		inline static unsigned long get(lua_State* L, unsigned long idx)
		{
			if (lua_isnumber(L, idx) || lua_isstring(L, idx))
			{
		...
		inline static void put(lua_State* L, const unsigned long& t)
		{
			lua_pushinteger(L, t);
		}
	};
	...
	public:
			...
			: m_state(state)
		{
			...
			if (state)
			{
				...
				luaL_mysetfuncs(state, destructor, 0);
				if (functions)
				{
					luaL_mysetfuncs(state, functions, 0);
				}
				lua_pop(state, 1);
		...
		inline LuaClass<TCLASS>& ctor(const char* name = "new")
		{
			...
			luaL_mysetfuncs(m_state, constructor, 0);
	...
	public:
		...
		inline LuaModule& fun(const char* name, F f)
		{
			...
			*funPtr = f;
			...
			luaL_mysetfuncs(m_state, regtab, 1);
		...
		inline LuaModule& fun(const char* name, lua_CFunction f)
		{
			...
			luaL_mysetfuncs(m_state, regtab, 0);
			lua_setglobal(m_state, m_moduleName.c_str());
#else
