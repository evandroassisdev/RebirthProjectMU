...
void CItemManager::Load(char* path) // OK
{
	...
	for (pugi::xml_node Category = ItemList.child("Category"); Category; Category = Category.next_sibling())
	{
		int section = Category.attribute("Index").as_int();

		for (pugi::xml_node Item = Category.child("Item"); Item; Item = Item.next_sibling())
		{
			ITEM_INFO info;

			memset(&info, 0, sizeof(info));

			info.Index = SafeGetItem(GET_ITEM(section, Item.attribute("Index").as_int()));
			strcpy_s(info.Name, Item.attribute("Name").as_string());
			info.Slot = Item.attribute("Slot").as_int();
			info.Skill = Item.attribute("SkillIndex").as_int();
			info.Width = Item.attribute("Width").as_int();
			info.Height = Item.attribute("Height").as_int();
			info.HaveSerial = Item.attribute("Serial").as_int();
			info.HaveOption = Item.attribute("Option").as_int();
			info.DropItem = Item.attribute("Drop").as_int();

			if (section >= 0 && section <= 5)
			{
				info.TwoHand = Item.attribute("TwoHand").as_int();
				info.Level = Item.attribute("DropLevel").as_int();
				info.DamageMin = Item.attribute("DamageMin").as_int();
				info.DamageMax = Item.attribute("DamageMax").as_int();
				info.AttackSpeed = Item.attribute("AttackSpeed").as_int();
				info.Durability = Item.attribute("Durability").as_int();
				info.MagicDurability = Item.attribute("MagicDurability").as_int();
				info.MagicDamageRate = Item.attribute("MagicDamageRate").as_int();
				info.RequireLevel = Item.attribute("RequireLevel").as_int();
				info.RequireStrength = Item.attribute("RequireStrength").as_int();
				info.RequireDexterity = Item.attribute("RequireDexterity").as_int();
				info.RequireEnergy = Item.attribute("RequireEnergy").as_int();
				info.RequireVitality = Item.attribute("RequireVitality").as_int();
				info.RequireLeadership = Item.attribute("RequireLeadership").as_int();
			...
			{
				info.Level = Item.attribute("DropLevel").as_int();
				info.Defense = Item.attribute("Defense").as_int();

				if (section == 6)
				{
					info.DefenseSuccessRate = Item.attribute("DefenseSuccessRate").as_int();
				}
				else if (section >= 7 && section <= 9)
				{
					info.MagicDefense = Item.attribute("MagicDefense").as_int();
				}
				else if (section == 10)
				{
					info.AttackSpeed = Item.attribute("AttackSpeed").as_int();
				...
				{
					info.WalkSpeed = Item.attribute("WalkSpeed").as_int();
				}
			...
			else if (section == 12)
			{
				...
				info.RequireDexterity = Item.attribute("RequireDexterity").as_int();
				info.RequireLeadership = Item.attribute("RequireLeadership").as_int();
				info.BuyMoney = Item.attribute("Money").as_int();
			}
			else if (section == 13)
			{
				info.Level = Item.attribute("RequireLevel").as_int();
				info.Durability = Item.attribute("Durability").as_int();
				info.Resistance[0] = Item.attribute("IceRes").as_int();
				info.Resistance[1] = Item.attribute("PoisonRes").as_int();
				info.Resistance[2] = Item.attribute("LightRes").as_int();
				info.Resistance[3] = Item.attribute("FireRes").as_int();
				info.Resistance[4] = Item.attribute("EarthRes").as_int();
				info.Resistance[5] = Item.attribute("WindRes").as_int();
				info.Resistance[6] = Item.attribute("WaterRes").as_int();

				info.RequireLevel = info.Level;
			}
			else if (section == 14)
			{
				info.Value = Item.attribute("Money").as_int();
				info.Level = Item.attribute("DropLevel").as_int();
			...
			info.AllowDrop = Item.attribute("Dump").as_bool();
			info.AllowSell = Item.attribute("SellToNPC").as_bool();
			info.AllowTrade = Item.attribute("Transaction").as_bool();
			info.AllowVault = Item.attribute("StoreWarehouse").as_bool();
