...
void PostMessage1(char* name,char* message,char* text) // OK
{
	...
	// ANCHOR_POST_FIX_START - was: wsprintf(buff,message,text); message is "%s: %s" (name, text)
	// but only text was passed, so the 2nd %s read garbage off the stack ("test: ÁN"-style junk).
	// pMsg.name below already carries the sender name separately - just copy text as-is.
...
void PostMessage2(char* name,char* message,char* text) // OK
{
	...
	// ANCHOR_POST_FIX2_START - was: wsprintf(&buff[1],message,text); same bug/fix as PostMessage1
	// above (message expects two %s, only text was given) - pMsg.name carries the sender name.
...
void PostMessage3(char* name,char* message,char* text) // OK
{
	...
	// ANCHOR_POST_FIX3_START - same bug/fix as PostMessage1/2 above.
	strncpy(&buff[1],text,sizeof(buff)-2);
...
void PostMessage4(char* name,char* message,char* text) // OK
{
	...
	// ANCHOR_POST_FIX4_START - same bug/fix as PostMessage1/2/3 above.
	strncpy(&buff[1],text,sizeof(buff)-2);
