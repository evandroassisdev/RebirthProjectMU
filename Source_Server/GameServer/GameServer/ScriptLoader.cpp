...
// __iob_func() to get at stdin/stdout/stderr. legacy_stdio_definitions.lib
// (linked in stdafx.h) covers most of that gap but doesn't provide this
// specific symbol on this toolset - so provide it ourselves. Lua only ever
// touches stdout/stderr through it (io/print/error reporting), so a static
// snapshot of the 3 standard streams is enough.
extern "C"
{
	FILE _lua_iob[3] = { *stdin, *stdout, *stderr };
...
void CScriptLoader::Load(char* path) // OK
{
	...
	if (luaL_loadfile(lua, path) != 0)
	{
		...
		gLog.Output(LOG_GENERAL, "Error: %s", lua_tostring(lua, -1));
		lua_close(lua);
...
void CScriptLoader::OnReadScript() // OK
{
	...
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnReadScript' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnShutScript() // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnShutScript' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnTimerThread() // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnTimerThread' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
int CScriptLoader::OnCommandManager(int aIndex, int code, char* arg) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCommandManager' is not defined.");
		this->m_critical.unlock();
		return 0;
	}
	...
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnCommandManager' must return a value.");
		this->m_critical.unlock();
		return 0;
	}
...
void CScriptLoader::OnCommandDone(int aIndex, int code) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCommandDone' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnCharacterEntry(int aIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCharacterEntry' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnCharacterClose(int aIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCharacterClose' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
int CScriptLoader::OnNpcTalk(int aIndex, int bIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnNpcTalk' is not defined.");
		this->m_critical.unlock();
		return 0;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnNpcTalk' must return a value.");
		this->m_critical.unlock();
		return 0;
	}
...
void CScriptLoader::OnMonsterDie(int aIndex, int bIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnMonsterDie' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnUserDie(int aIndex, int bIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnUserDie' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
void CScriptLoader::OnUserRespawn(int aIndex, int KillerType) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnUserRespawn' is not defined.");
		this->m_critical.unlock();
		return;
	}
...
int CScriptLoader::OnCheckUserTarget(int aIndex, int bIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCheckUserTarget' is not defined.");
		this->m_critical.unlock();
		return 1;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnCheckUserTarget' must return a value.");
		this->m_critical.unlock();
		return 1;
	}
...
int CScriptLoader::OnCheckUserKiller(int aIndex, int bIndex) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnCheckUserKiller' is not defined.");
		this->m_critical.unlock();
		return 1;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnCheckUserKiller' must return a value.");
		this->m_critical.unlock();
		return 1;
	}
...
int CScriptLoader::OnUserItemPick(int aIndex, int slot) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnUserItemPick' is not defined.");
		this->m_critical.unlock();
		return 1;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnUserItemPick' must return a value.");
		this->m_critical.unlock();
		return 1;
	}
...
int CScriptLoader::OnUserItemDrop(int aIndex, int slot, int x, int y) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnUserItemDrop' is not defined.");
		this->m_critical.unlock();
		return 1;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnUserItemDrop' must return a value.");
		this->m_critical.unlock();
		return 1;
	}
...
int CScriptLoader::OnUserItemMove(int aIndex, int aFlag, int aSlot, int bFlag, int bSlot) // OK
{
	...
	if (lua_isfunction(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: Function 'OnUserItemMove' is not defined.");
		this->m_critical.unlock();
		return 1;
	}
	...
	if (lua_isnumber(this->m_luaState, -1) == 0)
	{
		gLog.Output(LOG_GENERAL, "Error: 'OnUserItemMove' must return a value.");
		this->m_critical.unlock();
		return 1;
	}
