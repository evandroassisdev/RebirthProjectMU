...
void CServerInfo::ReadItemInfo() // OK
{
	...
	gItemManager.Load(gPath.GetFullPath("Item\\Item.xml"));

	gItemDrop.Load(gPath.GetFullPath("Item\\ItemDrop.xml"));
...
void CServerInfo::ReadUtilInfo() // OK
{
	...
	//XML
	gGameMaster.Load(gPath.GetFullPath("Util\\GameMaster.xml"));

	gNotice.Load(gPath.GetFullPath("Util\\Notice.xml"));
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_MonsterAttackRate = GetPrivateProfileInt(section,"MonsterAttackRate",100,path);

	this->m_MonsterDefenseCapPercent = GetPrivateProfileInt(section,"MonsterDefenseCapPercent",0,path);
