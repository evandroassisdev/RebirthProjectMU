...
int LuaGetObjectHardwareId(lua_State* L) // OK
{
	...
	// NOTE: this project doesn't track a hardware/HWID string on OBJECT.
	lua_pushstring(L, "");
	return 1;
}
...
int LuaGetObjectPassword(lua_State* L) // OK
{
	...
	// NOTE: this project doesn't keep the account password in the live OBJECT
	// struct (it's not needed post-login, and exposing it to scripts would be
	// a bad idea anyway).
	lua_pushstring(L, "");
	return 1;
}
...
int LuaCommandCheckGameMasterLevel(lua_State* L) // OK
{
	...
	lua_pushinteger(L, gGameMaster.CheckGameMasterLevel(&gObj[aIndex], aValue) ? 1 : 0);
...
int LuaCommandSend(lua_State* L) // OK
{
	...
	gCommandManager.ManagementCore(&gObj[aIndex], (char*)aString, -1);
...
int LuaFireworksSend(lua_State* L) // OK
{
	...
	GCFireworksSend(&gObj[aIndex], (aValue == -1) ? gObj[aIndex].X : aValue, (bValue == -1) ? gObj[aIndex].Y : bValue);
...
int LuaItemDrop(lua_State* L) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(dValue, &gObj[aIndex], gObj[aIndex].Map, aValue, bValue);
...
int LuaItemGive(lua_State* L) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(aValue, &gObj[aIndex], gObj[aIndex].Map, gObj[aIndex].X, gObj[aIndex].Y);
...
int LuaLogPrint(lua_State* L) // OK
{
	...
	gLog.Output(LOG_GENERAL, "%s", aString);
...
int LuaMasterSkillTreeRebuild(lua_State* L) // OK
{
	...
	// NOTE: this project's CMasterSkillTree has no reset/rebuild method - it
	// only tracks per-node values via InsertOption, there's nothing to bulk
	// reset from here. Left unimplemented rather than guessing at one.
	return 0;
}
...
int LuaMessageGet(lua_State* L) // OK
{
	...
	strcpy_s(value, sizeof(value), gMessage.GetMessage(aValue));
...
int LuaMonsterSummonCreate(lua_State* L) // OK
{
	...
	gObj[index].Attribute = 100;
	gObj[index].TargetNumber = -1;
	memset(&gObj[index].ActionState, 0, sizeof(gObj[index].ActionState));
...
int LuaMessageSend(lua_State* L) // OK
{
	...
	GCNewMessageSend(&gObj[aIndex], (char*)aString);
...
int LuaMessageSendToAll(lua_State* L) // OK
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			GCNewMessageSend(&gObj[n], (char*)aString);
		}
	}
...
int LuaObjectAddCoin(lua_State* L) // OK
{
	...
	GDSetCoinSend(aIndex, aValue, bValue, cValue, "Lua");
...
int LuaPermissionCheck(lua_State* L) // OK
{
	...
	// NOTE: reuses OBJECT::Quest[50], which nothing else in this project reads
	// or writes - it's not saved/loaded across logins, so these flags are
	// in-memory-only for now.
	lua_pushinteger(L, gObj[aIndex].Quest[aValue]);
...
int LuaPostSend(lua_State* L) // OK
{
	...
	// NOTE: bValue (the original pack's attachment/coin type) has no
	// equivalent here - this project's PostMessageN take (name,message,text)
	// only, so it's read from Lua for API compatibility but otherwise unused.
	(void)bValue;

	switch (aValue)
	{
	case 0:
		PostMessage1((char*)aString, gMessage.GetMessage(69), (char*)bString);
		break;
	case 1:
		PostMessage2((char*)aString, gMessage.GetMessage(69), (char*)bString);
		break;
	case 2:
		PostMessage3((char*)aString, gMessage.GetMessage(69), (char*)bString);
		break;
	case 3:
		PostMessage4((char*)aString, gMessage.GetMessage(69), (char*)bString);
		break;
	}
